  ...
  return (
    <div className="accountView">
      <div className="md:flex align-middle">
        <div className="profilePhoto" style={{ backgroundImage: `url(${API_URL}/users/profile/${accountInfo.profilePhoto}?token=${token})` }}>
        </div>
        <div className="details ml-8">
          <div className="names flex">{accountInfo.firstName} {accountInfo.lastName}

            {accountInfo.accountVerification?.verificationStatus === "VERIFIED" && <img src={verified} className="ml-2" alt="verified" />}
          </div>
          <div className="other-details">{accountInfo.email}</div>
          <div className="flex">
            <div className="other-details">{accountInfo.maritalStatus}</div>
            <div className="other-details ml-auto">{accountInfo.gender}</div>
          </div>
          <div className="flex">
            <div className="other-details">{new Date(accountInfo.dateOfBirth).toLocaleDateString()}</div>
